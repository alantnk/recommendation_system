...
def user_preferences(user_id: int) -> Any:
    user_preferences_obj = get_object_or_404(UserMoviePreferences, user_id=user_id)
    serializer = PreferencesSerializer(user_preferences_obj.preferences)
    return serializer.data


def user_watch_history(user_id: int) -> dict[Any, Any]:
    user_preferences_obj = get_object_or_404(UserMoviePreferences, user_id=user_id)
    return {"watch_history": user_preferences_obj.watch_history}
...
def add_preference(user_id: int, new_preferences: Dict[str, Any]) -> None:
    ...
    with transaction.atomic():
        ...
        current_preferences = defaultdict(list, user_preferences_obj.preferences)
        ...
        user_preferences_obj.preferences = dict(current_preferences)
        user_preferences_obj.save()
...
def add_watch_history(user_id: int, movie_id: int) -> None:
    ...
    :param user_id: ID of the user
    :param movie_id: ID of the movie watched
    """
    ...
    if not created:
        ...
        current_watch_history = user_preferences_obj.watch_history
        current_watch_history.append(movie_info)
        user_preferences_obj.watch_history = current_watch_history
        user_preferences_obj.save()
